...
function makePathOnRoad(path) {
  ...
  var tmpPath = [];
	for (destIdx = 0; destIdx < path.length -1 ; destIdx++){
        ...
        if (destIdx != 0 && calcDistacne(tmpPath[0], snapPath[snapPath.length-1]) > calcDistance(tmpPath[tmpPath.length-1], snapPath[snapPath.length-1])) {
          console.log("Not equal!");
          tmpPath.reverse();
        } 
				snapPath.push(tmpPath);
...
function calcDistance(src dest) {
  ...
  var tmp = tmp + Math.pow(src.lng() - dest.lng(), 2);
