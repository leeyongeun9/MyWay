...
function makePathOnRoad(path) {
  ...
  request.origin = path.getAt(0);
  request.destination = path.getAt(path.getLength()-1);
  request.travelMode = google.maps.DirectionsTravelMode.DRIVING;
  for (idx = 1 ; idx < path.getLength() - 1 ; idx++){
    request.waypoints = request.waypoints.concat(path.getAt(idx));
  }
  service.route(request, function (result, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      tmpPath = result.routes[0].overview_path;
      var a = tmpPath[0].lat();
      console.log("current destIdx is : " + destIdx);
